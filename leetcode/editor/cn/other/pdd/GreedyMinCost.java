    ...
    private static void GreedyMinCost(int phoneNumSize, int neededRepeatTimes, char[] phoneNum) {
        ...
        for (int currentNum = 0; currentNum < 10; currentNum++) {
            ...
            while (restNeededRepeatTimes > 0) {
                ...
                if (upperLimitNum < 10) {
                    if (repeatTimes[upperLimitNum] >= restNeededRepeatTimes) {
                        currentNumCost += (upperLimitNum - currentNum) * restNeededRepeatTimes;
                        restNeededRepeatTimes = 0;
                    } else {
                        currentNumCost += (upperLimitNum - currentNum) * repeatTimes[upperLimitNum];
                        restNeededRepeatTimes -= repeatTimes[upperLimitNum];
                    }
                }
                //如果手机号中数字比最佳数字小的情况就从后往前改
                if (lowerLimitNum >= 0) {
                    if (repeatTimes[lowerLimitNum] >= restNeededRepeatTimes) {
                        currentNumCost += (currentNum - lowerLimitNum) * restNeededRepeatTimes;
                        restNeededRepeatTimes = 0;
                    } else {
                        currentNumCost += (currentNum - lowerLimitNum) * repeatTimes[lowerLimitNum];
                        restNeededRepeatTimes -= repeatTimes[lowerLimitNum];
                    }
                }
                //扩大上下限
                upperLimitNum++;
